...
const MoonPost = (props = {}) => {
  ...
  // created
  const createdAt = new Date(item.createdAt) > new Date() ? new Date() : new Date(item.createdAt);
              ...
              { getLinks().map((l) => {
                ...
                return (
                  <Tooltip key={ l.ref } title={ `${l.type}: ${l.name}` }>
                    ...
                    } } onClick={ () => desktop.findOrCreateTask(l) }>
                      { l.name }
                    </Box>
                  </Tooltip>
                );
